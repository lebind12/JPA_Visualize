...
const SCENARIO_ID = 'lock.pessimistic-stock';
const BAD_PRODUCT_ID = 3;
const FIXED_PRODUCT_ID = 4;
const DEFAULT_THREADS = 5;
const DEFAULT_QUANTITY = 1;
const DEFAULT_RESET_STOCK = 100;

interface PessimisticStockDetailProps {
...
export default function PessimisticStockDetail({ scenario }: PessimisticStockDetailProps) {
      ...
      <section className="flex flex-col gap-4">
        <header className="flex flex-col gap-1">
          ...
          <p className="text-xs text-muted-foreground">
            ...
            Fixed는 <strong>SELECT ... FOR UPDATE</strong>로 행을 선점해 대기 후 직렬화합니다.
          </p>
        </header>

        <p className="text-xs text-muted-foreground">
          BAD는 상품 #{BAD_PRODUCT_ID}, FIXED는 상품 #{FIXED_PRODUCT_ID}를 사용합니다 — 독립된 row로 간섭 배제.
        </p>
