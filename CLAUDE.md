...
- ※ 이전 규칙 "최상단에 '구현 코드' 섹션 필수"는 철회한다. 아래 섹션 순서를 따른다.
- **권장 섹션 순서** (import 이후):
  1. `## {개념명}이란?` (또는 `## 정의`) — 필수, 파일 최상단
  2. `## 과정` — 권장(시각화 있을 때). 시각화 없으면 생략 가능
  3. `## 한줄 요약` — 필수
  4. `## 구현 코드` — 필수. `### BAD` / `### FIXED` 펜스드 java 블록. 파일 경로 주석 첫 줄 유지. `...` 생략 허용
  5. `## 건드리는 도메인` — DomainDiagram
  6. `## Bad 쪽에서 벌어지는 일`
  7. `## Fixed 쪽에서 벌어지는 일`
  8. `## 참고` — 외부 링크
