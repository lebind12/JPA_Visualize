...
| 6-A   | 랜딩 카드·도메인 카피 수치를 레지스트리 파생으로 정합 (meta 배제, 12 고정)                                             | ✅               |
| 6-B   | Hero 카피 · Bento hover · Stats 줄 · Tech Stack · Footer · 다크모드 QA                                                | 📋 **WIP**      |
| 6-C   | `/lab/concurrency` 본체 구현 — 락 상세 페이지와 기능 중복으로 **개발 보류 결정**                                       | ⏸️ 보류         |
| 7-A   | Testcontainers 인프라 (singleton `mysql:8.4`·AbstractIntegrationTest·test 프로필) + `nplus1.order-list` 회귀(Bad 41 / Fixed 2) | ✅               |
| 7-B   | 나머지 11개 시나리오 회귀 테스트 (락 3개는 단발 경로 우선, 동시성은 별도 전략)                                          | 📋 **WIP**      |
| +     | [커버리지 조사 문서](./docs/research/jpa-coverage-survey.md) · Gap Top 7 (벌크 캐시 · MultipleBagFetch · Jackson N+1 등) | 📋 **WIP**      |
...
- [x] 랜딩 수치 정합: 도메인 "6개 엔티티 · 6개 연관관계", 데모 "4개 카테고리 · 12 시나리오", 카드 4×"3 시나리오"
...
- [x] `./gradlew test` 통과 — `OrderListScenarioIntegrationTest` 3건 green (Bad 41 · Fixed 2 쿼리)
- [ ] 나머지 11개 시나리오 회귀 테스트 (Phase 7-B, WIP)
- [ ] `/lab/concurrency` 전용 통합 비교 Lab — **개발 보류** (락 상세와 중복)
